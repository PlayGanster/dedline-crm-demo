...
import { ArrowLeft, User, Mail, Phone, Save, MapPin, Briefcase, ShieldCheck, ShieldX, CheckCircle, Upload, X, FileBadge } from "lucide-react";
...
export default function PerformerEditPage() {
  ...
  const [formData, setFormData] = useState({
    last_name: mockPerformer.last_name,
    first_name: mockPerformer.first_name,
    middle_name: mockPerformer.middle_name,
    email: mockPerformer.email,
    phone: mockPerformer.phone,
    city: mockPerformer.city,
    source: mockPerformer.source as "CRM" | "APP",
    is_active: mockPerformer.is_active,
    is_verified: mockPerformer.is_verified,
    professions: mockPerformer.professions,
    passport_series: mockPerformer.passport_series,
    passport_number: mockPerformer.passport_number,
    passport_issued_by: mockPerformer.passport_issued_by,
    passport_issued_date: mockPerformer.passport_issued_date,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      router.push(`/dashboard/performers/${id}`);
    }, 1000);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <Button
          variant="outline"
          size="icon-sm"
          onClick={() => router.push(`/dashboard/performers/${id}`)}
        >
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-xl font-bold">Редактирование исполнителя</h1>
