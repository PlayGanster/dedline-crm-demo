...
export default function InvoiceCreatePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    clientKpp: "",
    dueDate: "",
    items: [{ name: "", quantity: 1, unit: "шт", price: 0 }] as InvoiceItem[],
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg sm:text-xl font-bold">Создание счёта</h1>
          <p className="text-xs sm:text-sm text-slate-500">Выставление нового счёта</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold">Реквизиты клиента</CardTitle>
          </CardHeader>
        ...
        <div className="flex items-center justify-end gap-2 pt-4 border-t">
          <Button type="button" variant="outline" size="sm" onClick={() => router.push("/dashboard/finance/invoices")}>
            Отмена
          </Button>
