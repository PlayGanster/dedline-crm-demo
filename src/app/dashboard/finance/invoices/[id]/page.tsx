...
import { ArrowLeft, Download, Print, Send, FileText, Building2, Calendar, User } from "lucide-react";
...
const mockInvoice = {
  ...
  clientKpp: "770101001",
  clientAddress: "123456, г. Москва, ул. Ленина, д. 1",
  amount: 45000,
  status: "paid",
  items: [
...
export default function InvoiceDetailPage() {
  ...
  const handleDownload = () => {
    const content = `СЧЁТ НА ОПЛАТУ №${mockInvoice.number} от ${mockInvoice.date}\n\nПОЛУЧАТЕЛЬ: ${mockInvoice.client}\nИНН: ${mockInvoice.clientInn}\nКПП: ${mockInvoice.clientKpp}\nАдрес: ${mockInvoice.clientAddress}\n\nИТОГО: ${formatMoney(mockInvoice.amount)}\n\nСрок оплаты до: ${mockInvoice.dueDate}\nСтатус: ${statusLabels[mockInvoice.status]}`;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg sm:text-xl font-bold">Счёт №{mockInvoice.number}</h1>
            <p className="text-xs sm:text-sm text-slate-500">от {mockInvoice.date}</p>
          </div>
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex justify-between">
            <span className="text-sm text-slate-500">Адрес</span>
            <span className="text-sm font-medium">{mockInvoice.clientAddress}</span>
          </div>
        </CardContent>
      </Card>
